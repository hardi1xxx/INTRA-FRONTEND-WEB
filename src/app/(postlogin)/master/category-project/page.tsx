...
import { DELETE_CATEGORY_PROJECT, EXPORT_CATEGORY_PROJECT, GET_CATEGORY_PROJECT, UPDATE_STATUS_CATEGORY_PROJECT } from "@/lib/redux/types";
...
import { UpsertCategoryProjectRequest } from "./schema";
...
export default function Page() {
  ...
  const [selectedData, setSelectedData] = useState<UpsertCategoryProjectRequest & WithId>();
 
  const { data, dataTotal, fetching, params, fetchingExport, resetTable, isFiltered } = useSelector((state: RootState) => state.categoryProject);
  ...
  useEffect(() => {
    return () => {
      ...
      dispatch(categoryProjectActions.resetParam());
    };
  }, [dispatch]);

  useEffect(() => {
    if (isFiltered && (params.start > 0 && data.length === 0)) {
      dispatch(categoryProjectActions.setParams({ ...params, start: params.start - params.length }));
  ...
  const onExportButtonClick = () => {
    dispatch({ type: EXPORT_CATEGORY_PROJECT });
  };

  const handleToggleChange = (id: number, checked: boolean) => {
      dispatch({
        type: UPDATE_STATUS_CATEGORY_PROJECT,
  ...
  const onServerSidePropsChange: CustomDatagridProps["onServerSidePropsChange"] = (data) => {
    ...
    dispatch(categoryProjectActions.setParams(combined));
  ...
  return (
    <MainPage title="Master Category Project">
