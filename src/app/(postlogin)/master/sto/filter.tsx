...
import { GET_STO, GET_STO_DROPDOWN, GET_STO_FILTER } from "@/lib/redux/types";
...
const TableFilter = ({ setResetSearch }: FilterType) => {
  ...
  const { dropdownOptions, dropdownOptionsLoading, params } = useSelector((state: RootState) => state.sto);
  ...
  const schema = yup.object({
    ...
    area_code: yup.string().optional().nullable(),
    area_name: yup.string().optional().nullable(),
    branch_code: yup.string().optional().nullable(),
    branch_name: yup.string().optional().nullable(),
    sto_code: yup.string().optional().nullable(),
    sto_name: yup.string().optional().nullable(),
  });
  ...
  const onSubmit: SubmitHandler<yup.InferType<typeof schema>> = (data) => {
    dispatch(stoActions.setIsFiltered(true));
    dispatch(stoActions.setParams({ ...params, ...data, start: 0 }));
    dispatch({ type: GET_STO });
  };

  const onClear = () => {
    dispatch(stoActions.setIsFiltered(false));
    setResetSearch(true)
    reset();
    dispatch(stoActions.resetDropdownOptions());
    dispatch(stoActions.clearParams());
    dispatch(stoActions.receiveNo());
  };
            ...
            <Grid item xs={12} md={3}>
              <Box marginY={0}>
                <FilterAutoComplete
                  ...
                  options={(dropdownOptions.area_name ?? []).map((item) => item.value.toString())}
                  onInputChange={debounce(onInputChange("area_name"), 500)}
                  onClose={onClose("area_name")}
                />
              </Box>
            </Grid>
            {/* Area Code */}
            <Grid item xs={12} md={3}>
              <Box marginY={0}>
                <FilterAutoComplete
                  ...
                  options={(dropdownOptions.area_code ?? []).map((item) => item.value.toString())}
                  onInputChange={debounce(onInputChange("area_code"), 500)}
                  onClose={onClose("area_code")}
                />
              </Box>
            </Grid>
            {/* Branch Name */}
            <Grid item xs={12} md={3}>
              <Box marginY={0}>
                <FilterAutoComplete
                  ...
                  options={(dropdownOptions.branch_name ?? []).map((item) => item.value.toString())}
                  onInputChange={debounce(onInputChange("branch_name"), 500)}
                  onClose={onClose("branch_name")}
                />
              </Box>
            </Grid>
            {/* Branch Code */}
            <Grid item xs={12} md={3}>
              <Box marginY={0}>
                <FilterAutoComplete
                  ...
                  options={(dropdownOptions.branch_code ?? []).map((item) => item.value.toString())}
                  onInputChange={debounce(onInputChange("branch_code"), 500)}
                  onClose={onClose("branch_code")}
                />
              </Box>
            </Grid>
            {/* STO Name */}
            <Grid item xs={12} md={3}>
              <Box marginY={0}>
                <FilterAutoComplete
                  ...
                  label="STO Name"
                  loading={dropdownOptionsLoading}
                  error={errors["sto_name"]?.message}
                  options={(dropdownOptions.sto_name ?? []).map((item) => item.value.toString())}
                  onInputChange={debounce(onInputChange("sto_name"), 500)}
                  onClose={onClose("sto_name")}
                />
              </Box>
            </Grid>
            {/* STO Code */}
            <Grid item xs={12} md={3}>
              <Box marginY={0}>
                <FilterAutoComplete
                  ...
                  label="STO Code"
                  loading={dropdownOptionsLoading}
                  error={errors["sto_code"]?.message}
                  options={(dropdownOptions.sto_code ?? []).map((item) => item.value.toString())}
                  onInputChange={debounce(onInputChange("sto_code"), 500)}
                  onClose={onClose("sto_code")}
                />
              </Box>
            </Grid>
            {/* button action */}
