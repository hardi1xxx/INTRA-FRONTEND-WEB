...
import { DELETE_STO, EXPORT_STO, GET_STO, UPDATE_STATUS_STO } from "@/lib/redux/types";
...
import { UpsertSTORequest } from "./schema";
...
export default function Page() {
  ...
  const [selectedData, setSelectedData] = useState<UpsertSTORequest & WithId>();
 
  const { data, dataTotal, fetching, params, fetchingExport, resetTable, isFiltered } = useSelector((state: RootState) => state.sto);
  ...
  useEffect(() => {
    return () => {
      ...
      dispatch(stoActions.resetParam());
    };
  }, [dispatch]);

  useEffect(() => {
    if (isFiltered && (params.start > 0 && data.length === 0)) {
      dispatch(stoActions.setParams({ ...params, start: params.start - params.length }));
  ...
  const onServerSidePropsChange: CustomDatagridProps["onServerSidePropsChange"] = (data) => {
    ...
    dispatch(stoActions.setParams(combined));
