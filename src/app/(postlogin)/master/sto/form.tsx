...
import { CREATE_STO, UPDATE_STO } from "@/lib/redux/types";
import { WithId } from "@/type/services";
import { UpsertSTORequest, upsertSTOSchema } from "./schema";
...
const UpsertForm = ({ open, setOpen, data, ...props }: FormType) => {
  ...
  const { fetching } = useSelector((state: RootState) => state.sto);
  ...
  const {
    ...
    resolver: yupResolver(upsertSTOSchema),
    defaultValues: {
      sto_code: "",
      sto_name: "",
      description: "",
  ...
  const onSubmit: SubmitHandler<UpsertSTORequest> = (values) => {
    const payload = {
      ...values
    };
    ...
    if (data?.id) {
      ...
      dispatch({ type: CREATE_STO, payload });
    }
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={"1rem"} justifyContent={"center"}>
          <Grid item xs={12} md={12}>
            <TextControlWidget control={control} name="sto_code" label="STO Code*" type="text" maxLength={255} autoFocus disabled={props.readonly} labelOnTextField />
            <TextControlWidget control={control} name="sto_name" label="STO Name*" type="text" maxLength={255} disabled={props.readonly} labelOnTextField />
