...
const TableFilter = ({ setResetSearch }: FilterType) => {
  ...
  const { dropdownOptions, dropdownOptionsLoading, params } = useSelector((state: RootState) => state.mitra);

  const schema = yup.object({
    status: yup.string().optional().nullable(),
    name: yup.string().optional().nullable(),
    pic: yup.string().optional().nullable(),
  });
  ...
  const onSubmit: SubmitHandler<yup.InferType<typeof schema>> = (data) => {
    ...
    dispatch(mitraActions.setParams({ ...params, ...data, start: 0 }));
  ...
  const onClear = () => {
    ...
    dispatch(mitraActions.clearParams());
            ...
            <Grid item xs={12} md={3}>
              <Box marginY={0}>
                <FilterAutoComplete
                  ...
                  options={(dropdownOptions.pic ?? []).map((item) => item.value.toString())}
                  onInputChange={debounce(onInputChange("pic"), 500)}
                  onClose={onClose("pic")}
                />
              </Box>
            </Grid>
