...
import { DELETE_MITRA, EXPORT_MITRA, GET_MITRA, UPDATE_STATUS_MITRA } from "@/lib/redux/types";
...
import { UpsertMitraRequest } from "./schema";
...
export default function Page() {
  ...
  const [selectedData, setSelectedData] = useState<UpsertMitraRequest & WithId>();
 
  const { data, dataTotal, fetching, params, fetchingExport, resetTable, isFiltered } = useSelector((state: RootState) => state.mitra);

  useEffect(() => {
    return () => {
      dispatch(mitraActions.setIsFiltered(false));
      dispatch(mitraActions.resetDropdownOptions());
      dispatch(mitraActions.receiveNo());
      dispatch(mitraActions.resetParam());
    };
  }, [dispatch]);

  useEffect(() => {
    if (isFiltered && (params.start > 0 && data.length === 0)) {
      dispatch(mitraActions.setParams({ ...params, start: params.start - params.length }));
      dispatch({ type: GET_MITRA });
    }
  ...
  const onExportButtonClick = () => {
    dispatch({ type: EXPORT_MITRA });
  };

  const handleToggleChange = (id: number, checked: boolean) => {
      dispatch({
        type: UPDATE_STATUS_MITRA,
  ...
  const onServerSidePropsChange: CustomDatagridProps["onServerSidePropsChange"] = (data) => {
    ...
    dispatch(mitraActions.setParams(combined));
