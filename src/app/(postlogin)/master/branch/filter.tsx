...
const TableFilter = ({ setResetSearch }: FilterType) => {
  ...
  const { dropdownOptions, dropdownOptionsLoading, params } = useSelector((state: RootState) => state.branch);
  ...
  const onSubmit: SubmitHandler<yup.InferType<typeof schema>> = (data) => {
    ...
    dispatch(branchActions.setParams({ ...params, ...data, start: 0 }));
  ...
  const onClear = () => {
    ...
    dispatch(branchActions.clearParams());
