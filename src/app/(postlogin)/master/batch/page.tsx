...
import { DELETE_BATCH, EXPORT_BATCH, GET_BATCH, UPDATE_STATUS_BATCH } from "@/lib/redux/types";
...
import { UpsertBatchRequest } from "./schema";
...
export default function Page() {
  ...
  const [selectedData, setSelectedData] = useState<UpsertBatchRequest & WithId>();
 
  const { data, dataTotal, fetching, params, fetchingExport, resetTable, isFiltered } = useSelector((state: RootState) => state.batch);
  ...
  useEffect(() => {
    return () => {
      ...
      dispatch(batchActions.resetParam());
    };
  }, [dispatch]);

  useEffect(() => {
    if (isFiltered && (params.start > 0 && data.length === 0)) {
      dispatch(batchActions.setParams({ ...params, start: params.start - params.length }));
  ...
  const onServerSidePropsChange: CustomDatagridProps["onServerSidePropsChange"] = (data) => {
    ...
    dispatch(batchActions.setParams(combined));
