...
import { DELETE_SHIFT, EXPORT_SHIFT, GET_SHIFT, UPDATE_STATUS_SHIFT } from "@/lib/redux/types";
...
import { UpsertShiftRequest } from "./schema";
...
export default function Page() {
  ...
  const permission = usePermission("master/shift");

  const [openForm, setOpenForm] = useState<boolean>(false);

  const [selectedData, setSelectedData] = useState<UpsertShiftRequest & WithId>();
 
  const { data, dataTotal, fetching, params, fetchingExport, resetTable, isFiltered } = useSelector((state: RootState) => state.shift);
  ...
  useEffect(() => {
    return () => {
      ...
      dispatch(shiftActions.resetParam());
    };
  }, [dispatch]);

  useEffect(() => {
    if (isFiltered && (params.start > 0 && data.length === 0)) {
      dispatch(shiftActions.setParams({ ...params, start: params.start - params.length }));
  ...
  return (
    <MainPage title="Master Shift">
      <Table
        ...
        onPageChange={(page) => {
          dispatch(shiftActions.setParams({ ...params, start: (page - 1) * params.length }));
        ...
        onPageSizeChange={(pageSize) => {
          dispatch(shiftActions.setParams({ ...params, start: 0, length: pageSize }));
          if(dataTotal === 0 && data.length === 0) return;
          dispatch({ type: GET_SHIFT });
        }}
        /**
        ...
        onGlobalSearchChange={debounce(
          (search) => {
            dispatch(shiftActions.setParams({ ...params, start: 0, search }));
        ...
        onSortChange={(sort) => {
          dispatch(shiftActions.setParams({ ...params, order: sort.map((s) => `${s.column},${s.order}`).join(",") }));
          if(dataTotal === 0 && data.length === 0) return;
          dispatch({ type: GET_SHIFT });
        }}
      >
