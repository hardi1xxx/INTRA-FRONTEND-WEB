...
import { CREATE_REGIONAL, UPDATE_REGIONAL } from "@/lib/redux/types";
import { WithId } from "@/type/services";
import { UpsertRegionalRequest, upsertRegionalSchema } from "./schema";
...
type FormType = {
  ...
  data?: UpsertRegionalRequest & WithId;
  onUpsert?: () => void;
  readonly?: boolean;
};
...
const UpsertForm = ({ open, setOpen, data, ...props }: FormType) => {
  ...
  const { fetching } = useSelector((state: RootState) => state.regional);
  ...
  const {
    ...
    resolver: yupResolver(upsertRegionalSchema),
    defaultValues: {
      regional_code: "",
      regional_name: "",
      description: "",
  ...
  const onSubmit: SubmitHandler<UpsertRegionalRequest> = (values) => {
    const payload = {
      ...values
    };
    ...
    if (data?.id) {
      dispatch({
        type: UPDATE_REGIONAL,
        payload: { id: data.id, ...payload },
      });
    } else {
      dispatch({ type: CREATE_REGIONAL, payload });
    }
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={"1rem"} justifyContent={"center"}>
          <Grid item xs={12} md={12}>
            <TextControlWidget control={control} name="regional_code" label="Regional Code*" type="text" maxLength={255} autoFocus disabled={props.readonly} labelOnTextField />
            <TextControlWidget control={control} name="regional_name" label="Regional Name*" type="text" maxLength={255} disabled={props.readonly} labelOnTextField />
