...
import { DELETE_REGIONAL, EXPORT_REGIONAL, GET_REGIONAL, UPDATE_STATUS_REGIONAL } from "@/lib/redux/types";
...
import { UpsertRegionalRequest } from "./schema";
...
export default function Page() {
  ...
  const [selectedData, setSelectedData] = useState<UpsertRegionalRequest & WithId>();
 
  const { data, dataTotal, fetching, params, fetchingExport, resetTable, isFiltered } = useSelector((state: RootState) => state.regional);
  ...
  useEffect(() => {
    return () => {
      ...
      dispatch(regionalActions.resetParam());
    };
  }, [dispatch]);

  useEffect(() => {
    if (isFiltered && (params.start > 0 && data.length === 0)) {
      dispatch(regionalActions.setParams({ ...params, start: params.start - params.length }));
  ...
  const onExportButtonClick = () => {
    dispatch({ type: EXPORT_REGIONAL });
  };

  const handleToggleChange = (id: number, checked: boolean) => {
      dispatch({
        type: UPDATE_STATUS_REGIONAL,
  ...
  const onServerSidePropsChange: CustomDatagridProps["onServerSidePropsChange"] = (data) => {
    ...
    dispatch(regionalActions.setParams(combined));
  ...
  return (
    <MainPage title="Master Regional">
