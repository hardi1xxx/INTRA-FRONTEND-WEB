...
import { DELETE_STATUS_PROJECT, EXPORT_STATUS_PROJECT, GET_STATUS_PROJECT, UPDATE_STATUS_STATUS_PROJECT } from "@/lib/redux/types";
...
import { UpsertStatusProjectRequest } from "./schema";
...
export default function Page() {
  ...
  const [selectedData, setSelectedData] = useState<UpsertStatusProjectRequest & WithId>();
 
  const { data, dataTotal, fetching, params, fetchingExport, resetTable, isFiltered } = useSelector((state: RootState) => state.statusProject);
  ...
  useEffect(() => {
    return () => {
      ...
      dispatch(statusProjectActions.resetParam());
    };
  }, [dispatch]);

  useEffect(() => {
    if (isFiltered && (params.start > 0 && data.length === 0)) {
      dispatch(statusProjectActions.setParams({ ...params, start: params.start - params.length }));
  ...
  const onExportButtonClick = () => {
    dispatch({ type: EXPORT_STATUS_PROJECT });
  };

  const handleToggleChange = (id: number, checked: boolean) => {
      dispatch({
        type: UPDATE_STATUS_STATUS_PROJECT,
  ...
  const onServerSidePropsChange: CustomDatagridProps["onServerSidePropsChange"] = (data) => {
    ...
    dispatch(statusProjectActions.setParams(combined));
  ...
  return (
    <MainPage title="Master Status Project">
