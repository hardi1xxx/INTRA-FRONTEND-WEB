...
import { UpsertStatusProjectRequest, upsertStatusProjectSchema } from "./schema";
...
type FormType = {
  ...
  data?: UpsertStatusProjectRequest & WithId;
  onUpsert?: () => void;
  readonly?: boolean;
};
...
const UpsertForm = ({ open, setOpen, data, ...props }: FormType) => {
  ...
  const { fetching } = useSelector((state: RootState) => state.statusProject);
  ...
  const {
    ...
    resolver: yupResolver(upsertStatusProjectSchema),
    defaultValues: {
      status_code: "",
      status_name: "",
      description: "",
  ...
  const onSubmit: SubmitHandler<UpsertStatusProjectRequest> = (values) => {
    const payload = {
      ...values
    };
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={"1rem"} justifyContent={"center"}>
          <Grid item xs={12} md={12}>
            <TextControlWidget control={control} name="status_code" label="Status Project Code*" type="text" maxLength={255} autoFocus disabled={props.readonly} labelOnTextField />
            <TextControlWidget control={control} name="status_name" label="Status Project Name*" type="text" maxLength={255} disabled={props.readonly} labelOnTextField />
