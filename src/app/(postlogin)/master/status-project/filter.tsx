...
const TableFilter = ({ setResetSearch }: FilterType) => {
  ...
  const { dropdownOptions, dropdownOptionsLoading, params } = useSelector((state: RootState) => state.statusProject);

  const schema = yup.object({
    status: yup.string().optional().nullable(),
    status_code: yup.string().optional().nullable(),
    status_name: yup.string().optional().nullable(),
  });
  ...
  const onSubmit: SubmitHandler<yup.InferType<typeof schema>> = (data) => {
    dispatch(statusProjectActions.setIsFiltered(true));
    dispatch(statusProjectActions.setParams({ ...params, ...data, start: 0 }));
    dispatch({ type: GET_STATUS_PROJECT });
  };

  const onClear = () => {
    dispatch(statusProjectActions.setIsFiltered(false));
    setResetSearch(true)
    reset();
    dispatch(statusProjectActions.resetDropdownOptions());
    dispatch(statusProjectActions.clearParams());
    dispatch(statusProjectActions.receiveNo());
  };
            ...
            <Grid item xs={12} md={3}>
              <Box marginY={0}>
                <FilterAutoComplete
                  ...
                  error={errors["status_name"]?.message}
                  options={(dropdownOptions.status_name ?? []).map((item) => item.value.toString())}
                  onInputChange={debounce(onInputChange("status_name"), 500)}
                  onClose={onClose("status_name")}
                />
              </Box>
            </Grid>
            {/* Status Project Code */}
            <Grid item xs={12} md={3}>
              <Box marginY={0}>
                <FilterAutoComplete
                  control={controller_filter}
                  field="status_code"
                  label="Status Project Code"
                  loading={dropdownOptionsLoading}
                  error={errors["status_code"]?.message}
                  options={(dropdownOptions.status_code ?? []).map((item) => item.value.toString())}
                  onInputChange={debounce(onInputChange("status_code"), 500)}
                  onClose={onClose("status_code")}
                />
              </Box>
            </Grid>
            {/* button action */}
