...
import { DELETE_AREA, EXPORT_AREA, GET_AREA, UPDATE_STATUS_AREA } from "@/lib/redux/types";
...
import { UpsertAreaRequest } from "./schema";
...
export default function Page() {
  ...
  const [selectedData, setSelectedData] = useState<UpsertAreaRequest & WithId>();
 
  const { data, dataTotal, fetching, params, fetchingExport, resetTable, isFiltered } = useSelector((state: RootState) => state.area);
  ...
  useEffect(() => {
    return () => {
      ...
      dispatch(areaActions.resetParam());
    };
  }, [dispatch]);

  useEffect(() => {
    if (isFiltered && (params.start > 0 && data.length === 0)) {
      dispatch(areaActions.setParams({ ...params, start: params.start - params.length }));
  ...
  const onServerSidePropsChange: CustomDatagridProps["onServerSidePropsChange"] = (data) => {
    ...
    dispatch(areaActions.setParams(combined));
