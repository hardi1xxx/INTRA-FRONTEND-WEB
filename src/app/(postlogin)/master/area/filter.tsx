...
import { GET_AREA, GET_AREA_DROPDOWN, GET_AREA_FILTER } from "@/lib/redux/types";
...
const TableFilter = ({ setResetSearch }: FilterType) => {
  ...
  const { dropdownOptions, dropdownOptionsLoading, params } = useSelector((state: RootState) => state.area);
  ...
  const onSubmit: SubmitHandler<yup.InferType<typeof schema>> = (data) => {
    ...
    dispatch(areaActions.setParams({ ...params, ...data, start: 0 }));
    dispatch({ type: GET_AREA });
  };

  const onClear = () => {
    dispatch(areaActions.setIsFiltered(false));
    setResetSearch(true)
    reset();
    dispatch(areaActions.resetDropdownOptions());
    dispatch(areaActions.clearParams());
    dispatch(areaActions.receiveNo());
  };
