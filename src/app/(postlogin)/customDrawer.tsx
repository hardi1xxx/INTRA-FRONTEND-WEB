...
const CustomDrawer = ({ isCollapsed }: { isCollapsed: boolean }): any => {
  ...
  const generateDrawer = (menus: MenuListType[], isChild = false) =>
    menus.map((value) => {
      ...
      // Cek apakah menu memiliki child yang bisa diakses (exclude hidden children)
      const hasAccessibleChild = value.child
        ?.some((child) => menuAccess.some((access) => access?.includes?.(child.id)));
      ...
      const filteredChildren = (value.child || [])
        .filter((child) => menuAccess.some((access) => access === child.id || access.startsWith(child.id + "/")));

      if (menuAccess.some((access) => access === value.id || access?.startsWith?.(value.id + "/"))) {
        return (
          <Accordion key={value.id} id={value.id} disableGutters sx={{ boxShadow: "none", marginBottom: "8px" }} expanded={expanded[value.id] || false} onChange={handleChange(value.id)} className="accordion-custom">
            <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls={`${value.id}-content`} className={expanded[value.id] ? "selected-accordion" : ""} id={`${value.id}-header`}>
              <Box
                sx={{
                  display: "flex",
                  alignItems: "center",
            ...
            >
              {generateDrawer(filteredChildren, true)}
            </AccordionDetails>
          </Accordion>
        );
      } else {
