...
import { getFilterArea, getAreaDatatable, getDropdownArea } from "@/lib/services";
...
function* getFilterOptions(props: PayloadAction<any>) {
  try {
    ...
    const response: Awaited<ReturnType<typeof getFilterArea>> =
      yield getFilterArea({ ...params, ...props.payload });
...
export function* watchGetAreaAsync() {
  ...
  yield takeEvery(GET_AREA_FILTER, getFilterOptions);
