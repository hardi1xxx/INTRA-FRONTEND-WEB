...
import { getDropdownSTO, getFilterSTO, getSTODatatable } from "@/lib/services";
...
function* getFilterOptions(props: PayloadAction<any>) {
  try {
    ...
    const response: Awaited<ReturnType<typeof getFilterSTO>> =
      yield getFilterSTO({ ...params, ...props.payload });
...
export function* watchGetSTOAsync() {
  ...
  yield takeEvery(GET_STO_FILTER, getFilterOptions);
