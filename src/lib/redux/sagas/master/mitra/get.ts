...
import { getDropdownMitra, getFilterMitra, getMitraDatatable } from "@/lib/services";
...
function* getFilterOptions(props: PayloadAction<any>) {
  try {
    ...
    const response: Awaited<ReturnType<typeof getFilterMitra>> =
      yield getFilterMitra({ ...params, ...props.payload });
...
export function* watchGetMitraAsync() {
  ...
  yield takeEvery(GET_MITRA_FILTER, getFilterOptions);
