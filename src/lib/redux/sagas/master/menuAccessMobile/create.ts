...
import { DefaultServiceResponse, createMenuAccessMobile, getMenuAccessMobile } from "@/lib/services";
...
import { CREATE_MENU_ACCESS_MOBILE } from "@/lib/redux/types";

type AnyAction = { type: string, data: DataMenuAccessMobileRequest }

export function* createMenuAccessMobileSagas({ data }: AnyAction) {
    try {
        yield put(requestMenuAccessMobile())

        yield createMenuAccessMobile(data);
...
export function* watchCreateMenuAccessMobileAsync() {
    yield takeEvery(CREATE_MENU_ACCESS_MOBILE, createMenuAccessMobileSagas)
}