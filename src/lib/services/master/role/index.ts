...
export const exportExcelRoleService = (axios: AxiosInstance) => async (search: string): Promise<Blob> => {
    try {
        const response = await axios.post(`/master/role/excel-download-data-role`, { search }, {
            responseType: 'blob'
        })
...
export type RoleServiceType = {
    ...
    exportExcelRole: (search: string) => Promise<Blob>
}
