...
const AGGrid = ({ ...props }: AgGridReactProps & CustomAGGridProps) => {
  ...
  const onChangePageSize = (event: SelectChangeEvent<number>) => {
    ...
    setPage(1);
    props.gridRef.current!.api.setGridOption("paginationPageSize", event.target.value as number);
  };
  ...
  useEffect(() => {
    const totalPages = Math.ceil((props.totalData ?? 0) / (length ?? props.paginationPageSize ?? 10));
  ...
  return (
    <Box
      ...
      minHeight={props.minHeight ?? "380px"}
      sx={{
            ...
            <Grid container display="flex" alignItems="center" spacing={"1rem"}>
              <Grid item xs={12} display="flex" justifyContent={{ xs: "center", md: "start" }} alignItems="center">
                <Box>
                  ...
                  >
                    Total Data: {props.serverSideMode ? props.totalData : isReadyGrid ? props.gridRef.current!.api.getDisplayedRowCount() ?? 0 : 0}
                  </Typography>
                </Box>
              </Grid>
            </Grid>
          ...
          <Grid item xs={12} md={7}>
            <Box display={"flex"} justifyContent={{ xs: "center", md: "end" }} width={"100%"} gap={"1rem"}>
              <StyledPagination
                ...
                count={paginationTotalPage ?? Math.ceil((props.serverSideMode ? props.totalData : isReadyGrid ? props.gridRef.current!.api.getDisplayedRowCount() ?? 0 : 0) / (length ?? props.paginationPageSize ?? 10))}
                color="primary"
                shape="rounded"
